...
<body>
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        $volum = 0;
            ...
            case 'piramide':
                ...
                $altura = $_POST['altura'];
                $superficie_total = $base_larga * $base_ample + 2 * $base_larga * sqrt(pow($base_ample / 2, 2) + pow($altura, 2)) + 2 * $base_ample * sqrt(pow($base_larga,>                $volum = ($base_larga * $base_ample * $altura) / 3;
                break;
    ...
    <script>
        // Funció per mostrar les caselles corresponents segons la selecció de l'usuari
        document.getElementById("cuerpo").addEventListener("change", function() {
            var cuerpoSeleccionat = this.value;
            var dimensionesContainer = document.getElementById("dimensiones-container");
            dimensionesContainer.innerHTML = ""; // Netegem el contingut anterior

            // Mostrar les caselles corresponents al cos seleccionat
            switch (cuerpoSeleccionat) {
                case "cilindre":
